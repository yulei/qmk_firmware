...
static void usb_lld_serve_interrupt(USBDriver *usbp) {
  ...
  otgp->GINTSTS = sts;
  /*if (!(sts&GINTSTS_SOF))
    SEGGER_RTT_printf(0, "usb interrupt: 0x%x\r\n", sts);
    */
  ...
  if (sts & GINTSTS_USBRST) {
    ...
    //SEGGER_RTT_printf(0, "usb interrupt: reset\r\n");
    /* Preventing execution of more handlers, the core has been reset.*/
    return;
  }

  /* Wake-up handling.*/
  //SEGGER_RTT_printf(0, "usb interrupt: before wakeup\r\n");
  if (sts & GINTSTS_WKUPINT || ((usbp->state == USB_SUSPENDED) && ((sts&GINTSTS_OEPINT) || (sts&GINTSTS_IEPINT)))) {
    ...
    _usb_wakeup(usbp);
    //SEGGER_RTT_printf(0, "usb interrupt: wakeup\r\n");
  }
  ...
  if (sts & GINTSTS_USBSUSP) {
    ...
    _usb_suspend(usbp);
    //SEGGER_RTT_printf(0, "usb interrupt: suspend\r\n");
  }
