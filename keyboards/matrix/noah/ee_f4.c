...
static eef4_status_t switch_sector(void) {
    eef4_status_t  status = EE_SUCESS;
    static uint8_t flash_buf[EE_MAX_BYTES];
    ...
    memset(&flash_buf[0], 0xFF, EE_MAX_BYTES);
    while (end >= start + 4) {
        uint16_t addr = *(uint16_t *)end;
        if (addr < EE_MAX_BYTES) {
            // valid data
            if (flash_buf[addr] == 0xFF) {
                flash_buf[addr] = *(uint8_t *)(end + 3);
            }
        }
        end -= 4;
  }
  ...
  if (prepare_sector(next_sector) == EE_SUCESS) {
    ...
    for (uint32_t i = 0; i < EE_MAX_BYTES; i++) {
      if (flash_buf[i] != 0xFF) {
          data = (i << 16) | flash_buf[i];
          status = FLASH_ProgramWordF4(addr, data);
          if (status != EE_SUCESS) {
              return status;
          }
          addr += 4;
...
int32_t eef4_write_byte(uint32_t addr, uint8_t data) {
  ...
  return FLASH_ProgramWordF4(valid_addr, (addr << 16) | data);
}
