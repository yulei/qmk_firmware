...
#ifndef AdafruitBleResetPort
#define AdafruitBleResetPort GPIOA
#endif
#ifndef AdafruitBleResetPin
#define AdafruitBleResetPin 2
#endif

/*
#ifndef AdafruitBleCSPort
#define AdafruitBleCSPort GPIOA
#endif
#ifndef AdafruitBleCSPin
#define AdafruitBleCSPin 4
#endif
*/

#ifndef AdafruitBleIRQPort
#define AdafruitBleIRQPort GPIOA
#endif
#ifndef AdafruitBleIRQPin
#define AdafruitBleIRQPin 3
#endif
...
#define SdepWait 100 /* microseconds */
...
{
    uint8_t result = 0;
    SPI_exchange(&data, &result, sizeof(data));
    return result;
}
...
static bool sdep_send_pkt(const struct sdep_msg *msg, uint16_t timeout) {
  ...
  do {
    ready = SPI_send_byte(msg->type) != SdepSlaveNotReady;
    if (ready) {
      break;
    }
  ...
  if (ready) {
    ...
    SPI_send(&msg->cmd_low, sizeof(*msg) - (1 + sizeof(msg->payload)) + msg->len);
    success = true;
  }
...
static bool sdep_recv_pkt(struct sdep_msg *msg, uint16_t timeout) {
  ...
  do {
    ready = palReadPad(AdafruitBleIRQPort, AdafruitBleIRQPin);
    if (ready) {
      break;
    }
    wait_us(1);
  ...
  if (ready) {
    ...
    do {
      ...
      SPI_recv(&msg->cmd_low, sizeof(*msg) - (1 + sizeof(msg->payload)));

      // and get the payload if there is any
      if (msg->len <= SdepMaxPayload) {
        SPI_recv(msg->payload, msg->len);
      }
      success = true;
      break;
    } while (timer_elapsed(timerStart) < timeout);
...
static void resp_buf_read_one(bool greedy) {
  ...
  if (palReadPad(AdafruitBleIRQPort, AdafruitBleIRQPin)) {
    struct sdep_msg msg;
...
again:
    if (sdep_recv_pkt(&msg, SdepTimeout)) {
      ...
      if (greedy && resp_buf.peek(last_send) && palReadPad(AdafruitBleIRQPort, AdafruitBleIRQPin)){//digitalRead(AdafruitBleIRQPin)) {
        goto again;
      }
    }
...
static bool ble_init(void) {
  ...
  palSetPadMode(AdafruitBleIRQPort, AdafruitBleIRQPin, PAL_MODE_INPUT_PULLDOWN);

  SPI_init();

  // Perform a hardware reset
  palSetPadMode(AdafruitBleResetPort, AdafruitBleResetPin, PAL_MODE_OUTPUT_PUSHPULL);
  palSetPad(AdafruitBleResetPort, AdafruitBleResetPin);
  palClearPad(AdafruitBleResetPort, AdafruitBleResetPin);
  wait_ms(10);
  palSetPad(AdafruitBleResetPort, AdafruitBleResetPin);
...
void adafruit_ble_task(void) {
  ...
  if (resp_buf.empty() && (state.event_flags & UsingEvents) && palReadPad(AdafruitBleIRQPort, AdafruitBleIRQPin)) {
